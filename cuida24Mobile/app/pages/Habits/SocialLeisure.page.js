...
import { postSocLei } from '../../redux/actions/index'
import { connect } from 'react-redux';

class SocialLeisurePage extends React.Component {
  constructor(props) {
    super(props);
    this.state  = {
      token: '',
  ...
  async componentWillMount() {
    AsyncStorage.getItem('@login:')
      .then((token_res) => {
        AsyncStorage.getItem('@socialLeisure')
          .then((activities) => {
            this.setState({
              token: token_res,
          ...
          .catch((error) => {
            console.warn('AsyncStorage - getItem: socialLeisure', error);
          });
      })
      .catch((error) => {
  ...
  saveSocLei = () => {
    ...
    duration = duration[0];
    this.props.postSocLei({token, date, type, act, duration});
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={{ flexDirection: 'row' }}>
          ...
          <View style={styles.buttonItem}>
            <Button
              onPress={() => this.saveSocLei()}
              disabled={ !this.areBothSelected() }
              title="Guardar"
              titleStyle={styles.titles}
              buttonStyle={styles.buttonSave}
              accessibilityLabel="Submete a atividade social realizada."
            />
          </View>
        </View>
        <Button
          onPress={() => this.props.navigation.navigate('HistoricSocialLeisure')}
          title="Histórico"

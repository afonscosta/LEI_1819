...
export default class HistoricIndivLeisurePage extends React.Component {
  constructor(props) {
    super(props);
    this.state  = {
      token: '',
  ...
  async fetchIndivLeisures(token) {
    fetch("http://10.0.2.2:8000/cuida24/activity?type=LI", {
      method: 'GET',
      headers: {
          ...
          AsyncStorage.setItem(
            ...
            .then(() => {
              AsyncStorage.getItem('@durations')
                ...
                .catch((error) => {
                  console.warn('AsyncStorage - getItem: indivLeisures', error);
                });
            })
            .catch((error) => {
              console.warn('AsyncStorage - setItem: indivLeisures', error);
            });
        }
      })
      .catch((error) => {
        AsyncStorage.getItem('@durations')
          .then((durations) => {
            AsyncStorage.getItem('@indivLeisures')
              .then((activities) => {
                this.setState({
                  durations: JSON.parse(durations),
  ...
  onRefresh() {
    ...
    const fetchData = async () => {
      await this.fetchIndivLeisures(this.state.token);
    }
    fetchData().then(() => {
	...
	_listEmptyComponent = () => {
		return (
			<View style={styles.emptyList}>
				<Text>Não existe lazer individual registado.</Text>
			</View>
		)
	}

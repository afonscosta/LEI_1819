...
export default {
  ...
  computed:
  {
    ...
    summary () {
      ...
      if (small) {
        return this.translateMonthsSmall(this.calendar.start.format(this.formats.xs))
      }
      let large = this.$vuetify.breakpoint.mdAndUp
      var sum = this.calendar.summary(false, !large, false, !large)
      return this.translateMonths(sum.split(' '))
    },
    todayDate () {
  ...
  methods:
  {
    translateMonths (sumSplit) {
      if (sumSplit[0] === 'January') {
        sumSplit[0] = 'Janeiro'
      } else if (sumSplit[0] === 'February') {
        sumSplit[0] = 'Fevereiro'
      } else if (sumSplit[0] === 'March') {
        sumSplit[0] = 'Março'
      } else if (sumSplit[0] === 'April') {
        sumSplit[0] = 'Abril'
      } else if (sumSplit[0] === 'May') {
        sumSplit[0] = 'Maio'
      } else if (sumSplit[0] === 'June') {
        sumSplit[0] = 'Junho'
      } else if (sumSplit[0] === 'July') {
        sumSplit[0] = 'Julho'
      } else if (sumSplit[0] === 'August') {
        sumSplit[0] = 'Agosto'
      } else if (sumSplit[0] === 'September') {
        sumSplit[0] = 'Setembro'
      } else if (sumSplit[0] === 'October') {
        sumSplit[0] = 'Outubro'
      } else if (sumSplit[0] === 'November') {
        sumSplit[0] = 'Novembro'
      } else if (sumSplit[0] === 'December') {
        sumSplit[0] = 'Dezembro'
      }
      if (sumSplit[1]) {
        return sumSplit[0] + ' ' + sumSplit[1]
      }
      return sumSplit[0]
    },
    translateMonthsSmall (sum) {
      if (sum === 'Feb') {
        sum = 'Fev'
      } else if (sum === 'Apr') {
        sum = 'Abr'
